...
class Provider(ProviderBase):
    ...
    def __init__(self, name, context, params, query_interface):
        ...
        super(Provider, self).__init__(
            name, context, 'git', True, query_interface)
        ...
        self.temp_dir = path.join(
            context.library_cache_path, ensure_valid_pathname(self.host))
    ...
    def find_package(self, pkg_name, pkg_version, check_valid, refresh):
        ...
        if check_remote and not repo.is_valid_remote_branch(pkg_version):
            ...
            raise PackageNotFound(
                self, pkg_name, pkg_version, "Version does not exist")
    ...
    def get_package_info(self, pkg, refresh=True):
        ...
        vlog('Getting package info for %s-%s (refresh=%s)' %
             (pkg.name, pkg.version, str(refresh)))
        ...
        try:
            ...
            vlog('Failed to decode packing info JSON for "%s"\n%s' % (
                pkg.name, json))
            raise
