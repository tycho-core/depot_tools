...
class Workspace(object):
    ...
    def __init__(self, context, root_dir, check_valid=True, refresh_dependencies=True):
        ...
        self.__info = WorkspaceInfo.create_from_json_file(
            self.get_workspace_info_path())
    ...
    def save_local_workspace_info(self):
        ...
        for binding in self.__package_set.get_package_bindings():
            ...
            relpath = os.path.relpath(
                binding.get_package_root_dir(), self.root_dir)
            pkg_name = '%s:%s:%s' % (
                package.provider.source_name, package.name, package.version)
    ...
    def verify(self):
        ...
        if local_info:
            ...
            for binding in local_info.get_package_bindings():
                ...
                pkg = self.context.package_manager.get_package(
                    dep.source, dep.name, dep.branch)
                ...
                if not pkg:
                    ...
                    local_status = pkg.local_filesystem_status(
                        os.path.join(self.root_dir, local_dir))

                    if not local_status.is_installed():
                        pkg_status.append([
                            PkgDoesNotExistLocally,
                            "Package '%s' is not installed locally" % (
                                str(pkg))
                        ])
                        ...
                        pkg_status.append([
                            PkgModified,
                            "Package '%s' has been locally modified" % (
                                str(pkg))
                        ])
                    elif local_status.get_version() != dep.branch:
                        pkg_status.append([
                            PkgIncorrectVerson,
                            "Package '%s' is using locally using the incorrect version" % (
                                str(pkg))
                        ])
                    else:
                        pkg_status.append([
    ...
    def foreach(self, provider, args):
        ...
        for binding in self.__package_set.get_package_bindings():
            ...
            results.append((binding, execute(
                args[0], binding.get_package_root_dir(), args[1:], capture=True)))
        console.end_task()
        ...
        for result in results:
            console.write_line(
                '---------------------------------------------------')
            console.write_line(task_name + ' - ' +
                               result[0].get_package().name)
            console.write_line(
                '---------------------------------------------------')
    ...
    def get_available_imports(self):
        ...
        for project in projects:
            ...
            pdict['existing'] = existing_deps.project_exists(
                project.get_display_name())
            pdict['versions'] = [version.get_display_name()
    ...
    def import_project(self):
        ...
        hub_import = interactive_interface.select_import(
        ...
        self.context.console.write_line('Selected %s:%s:%s' % (
            provider.source_name, project.get_display_name(), version.get_display_name()))
    ...
    def add_command_line_options(parser):
        ...
        # init action
        init = add_command_line_action(
            subparsers, 'init', action_help='Initialize a new workspace')
        ...
        depends = add_command_line_action(subparsers, 'depends',
                                          action_help='Show dependency information for this workspace')
        ...
        # delete action
        add_command_line_action(subparsers, 'delete',
                                action_help='Delete the workspace')

        # status action
        status = add_command_line_action(subparsers, 'status',
                                         action_help='Show status of all repositories in the workspace')
